...
CREATE TABLE IF NOT EXISTS trabajadores (
  ...
  -- Bloqueo Optimista contra Dirty Reads
...
CREATE TABLE IF NOT EXISTS descansos_medicos (
  ...
  cie10_diagnostico_encrypted TEXT NOT NULL,
  -- AES-256 Ley N° 29733
...
CREATE INDEX IF NOT EXISTS idx_trabajadores_status ON trabajadores(status)
WHERE
  deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_descansos_fechas ON descansos_medicos(fecha_inicio, fecha_fin);
CREATE INDEX IF NOT EXISTS idx_descansos_trabajador ON descansos_medicos(id_trabajador)
WHERE
  deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_campamento_cama ON campamento_habitaciones(modulo_habitacion, cama_asignada)
WHERE
  deleted_at IS NULL;
-- 3. PROCEDIMIENTO CON BLOQUEO PESIMISTA (SELECT ... FOR UPDATE)
  ...
  CREATE
  OR REPLACE FUNCTION asignar_cama_campamento_transaccional(
    p_id_asignacion VARCHAR,
    p_id_trabajador VARCHAR,
    p_modulo VARCHAR,
    p_cama VARCHAR,
    p_fecha_ingreso DATE
  ) RETURNS BOOLEAN AS $ $ DECLARE v_cama_ocupada INT;
BEGIN -- Bloquear el registro especifico para evitar race condition
  PERFORM 1
FROM
  campamento_habitaciones
WHERE
  modulo_habitacion = p_modulo
  AND cama_asignada = p_cama
  AND fecha_salida IS NULL
  AND deleted_at IS NULL FOR
UPDATE;
SELECT
  COUNT(*) INTO v_cama_ocupada
FROM
  campamento_habitaciones
WHERE
  modulo_habitacion = p_modulo
...
IF v_cama_ocupada > 0 THEN RAISE EXCEPTION 'La cama % en módulo % ya fue asignada por otra sesión.',
  p_cama,
  p_modulo;
RETURN FALSE;
END IF;
