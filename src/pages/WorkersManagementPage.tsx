...
export const WorkersManagementPage: React.FC<WorkersManagementPageProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Batch Print View - 2 Large Badges per A4 page */}
      {isBatchPrinting && (
        <div className="fixed inset-0 z-[100] bg-white print:block print:bg-white overflow-y-auto print-only">
          {Array.from({ length: Math.ceil(filteredWorkers.length / 2) }).map((_, pageIndex) => {
            ...
            return (
              <div 
                key={pageIndex} 
                className="flex flex-col items-center justify-around py-8 px-4 mx-auto h-[290mm] min-h-[290mm] box-border"
                style={{ width: '210mm', pageBreakAfter: 'always', breakAfter: 'page' }}
              >
                {pair.map((w) => (
                  <div key={w.id} className="avoid-page-break avoid-break-inside flex justify-center items-center w-full my-auto">
